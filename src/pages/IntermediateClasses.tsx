...
const IntermediateClasses = () => {
  ...
  const { profUsers, interStudents } = useMemo(() => {
    ...
    return {
      ...
      interStudents: data.filter((u: any) => u.role === "student" && u.category === "intermediate"),
    };
  }, [data]);
  ...
  const intermediateDepts = (departments || []).filter((d: any) => d?.category === "intermediate");
  const professors        = profUsers || [];

  const filteredStudents = interStudents.filter((s: any) =>
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.class)                             { toast.error("Part (I or II) is required"); return false; }
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border shrink-0">
                <div>
                  <h2 className="font-display text-lg font-bold text-foreground">Add Intermediate Class</h2>
                  {generatedClassName && (
