...
const Faculty = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (form.firstName.trim().length > 30) {
      toast.error("First name must not exceed 30 characters");
      return false;
    }
    if (form.lastName.trim().length > 30) {
      toast.error("Last name must not exceed 30 characters");
      return false;
    }
    ...
    if (!/^(92\d{10}|0\d{10})$/.test(form.phone)) {
      toast.error(
        "Phone: 12 digits starting with 92, or 11 digits starting with 0",
      );
      return false;
    }
          ...
          <thead>
            <tr className="border-b border-border">
              {[
                ...
                "Last Name",
                "Date of Joining",
                ...(isProfessor ? ["HOD", "Department"] : []),
                "Action",
              ].map((h) => (
                <th
                  key={h}
                  className="table-header p-3 sm:p-4 text-left text-xs sm:text-sm font-medium text-muted-foreground"
                >
                  {h}
                </th>
              ))}
              ...
              >
                ...
                {isProfessor && (
                  <>
                    <td className="p-3 sm:p-4 text-xs sm:text-sm">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${m?.isHod ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                      >
                        {m?.isHod ? "Yes" : "No"}
                      </span>
                    </td>
