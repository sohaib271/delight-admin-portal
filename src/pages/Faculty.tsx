...
const Faculty = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (form.firstName.trim().length > 30) {
    ...
    if (!/^\d{13}$/.test(form.cnic)) {
      toast.error("CNIC must be exactly 13 digits"); return false;
    }
    ...
    if (!form.doj) {
      toast.error("Date of joining is required"); return false;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res?.error || res?.statusCode >= 400) {
        if (Array.isArray(res?.message?.errors)) {
          res.message.errors.forEach((e: string) => toast.error(e));
        } else {
    ...
    return (
      <div className="overflow-x-auto">
        <table className="w-full min-w-[600px]">
          ...
          <tbody>
            {displayData?.map((m, i) => (
              <motion.tr key={m?._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                {isProfessor && (
                  <>
                    <td className="p-4 text-sm">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${m?.isHod ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                        {m?.isHod ? "Yes" : "No"}
                      </span>
                    </td>
                    <td className="p-4 text-sm">{m?.department?.code}</td>
                  </>
                )}
                <td className="p-4">
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <span className="rounded-lg bg-primary/10 px-3 py-1.5 text-sm font-semibold text-primary w-fit">
          ...
          <motion.div key={section.type} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" className="gap-1.5" onClick={() => openModal(section.type)}>
        ...
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              // ✅ Fixed: max height + scroll so modal never overflows viewport
              ...
              <div className="overflow-y-auto px-6 py-4 flex-1">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Phone No">
                    <Input placeholder="03xxxxxxxxx or 92xxxxxxxxxx" value={form.phone} onChange={set("phone")} />
                  </Field>
                   <Field label="City">
                    <Input value={form.city} onChange={set("city")} />
                  </Field>
                  <Field label="CNIC (13 digits)">
                    <Input placeholder="3520012345678" value={form.cnic} onChange={set("cnic")} maxLength={13} />
                  </Field>
