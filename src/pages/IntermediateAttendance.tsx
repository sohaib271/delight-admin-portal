...
const IntermediateAttendance = () => {
  ...
  const allStudents = mockStudents.filter((s) => s.category === "intermediate");
  ...
  return (
    <div>
      <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4 font-display text-xl font-bold text-foreground sm:text-2xl">Intermediate Attendance</motion.h1>
          ...
          <tbody>
            {displayed.map((s, i) => (
              <motion.tr key={s.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="border-b border-border last:border-0 hover:bg-secondary/50 transition-colors">
                ...
                <td className="p-4 text-sm font-semibold">{s.total}</td>
              </motion.tr>
            ))}
            {displayed.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-sm text-muted-foreground">No records found</td></tr>}
          </tbody>
        </table>
      </motion.div>
...
export default IntermediateAttendance;
